...
const EmbedVideo = ({ video, defaultInfoState = false }: { video: videoType, defaultInfoState?: boolean }) => {
  ...
  return (
    <div className='relative h-full w-full' onClick={() => setShowInfo(!showInfo)}>
        ...
        <div className='flex flex-row justify-between gap-5'>
          ...
          <div>
            <a className='text-xs sm:text-base opacity-80 hover:opacity-50 underline cursor-pointer' target="_blank" href={video.url}>See original</a>
          </div>
        </div>
